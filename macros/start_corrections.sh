...
export FILELIST=/mnt/pool/nica/7/parfenovpeter/Soft/qntools_macros_femtodst/macros/lists/runlists_femtodst_auau_200gev.list
...
rsync -vuzh $MACRO_EXE ${TMP_DIR}/makeQvectors.h &>> $LOG
rsync -vuzh $MACRO_EXE ${TMP_DIR}/utils.h &>> $LOG
root -l -b -q $CONVERT_EXE'("'${IN_FILE}'","'${TMP_DIR}/stardata.root'")' &>> $LOG
cd $TMP_DIR
