...
export START_SCRIPT=/mnt/pool/nica/7/parfenovpeter/Soft/qntools_macros_femtodst/macros/start_aux_hadd.sh.template
...
find $INPUT_DIR -type f -name 'aux_*.root' | while read -r line
do
  export OUTPUT_FILE=${OUT_DIR}/lists/aux_part${ipart}.list
  if [[ ! -f $OUTPUT_FILE ]]
  then
    touch $OUTPUT_FILE
  fi
...
sed\
        ...
        $START_SCRIPT > ${OUT_DIR}/start_aux_hadd.sh
