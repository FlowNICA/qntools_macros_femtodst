...
export START_SCRIPT=/mnt/pool/nica/7/parfenovpeter/Soft/qntools_macros_femtodst/macros/start_corr_hadd.sh.template
...
find $INPUT_DIR -type f -name 'corr*.root' | while read -r line
do
  export OUTPUT_FILE=${OUT_DIR}/lists/corr_part${ipart}.list
  if [[ ! -f $OUTPUT_FILE ]]
  then
    touch $OUTPUT_FILE
  fi
...
sed\
        ...
        $START_SCRIPT > ${OUT_DIR}/start_corr_hadd.sh
